...
def get_frame(vid_stream, stream):
    ...
    if frame is None:
        ...
        frame = imutils.resize(frame, width=IMG_S)
        return frame
...
class Tracker:
    ...
    def track(self, frame):
        ...
        # Apply NMS
        detections = non_max_suppression(detections, CONF_THRES, IOU_THRES, classes=CLASSES, agnostic=False)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    #cv2.circle(frame, (int(x), int(y)), int(radius),
                     #  (0, 255, 255), 2)
                    #cv2.circle(frame, (int(self.midx), int(self.midy)), 5, (0, 0, 255), -1)

                    self.xoffset = int(xywh[0]+0.5*xywh[2] - self.midx)
                    self.yoffset = int(self.midy - xywh[1]-0.5*xywh[3])
                    radius = 0.5*xywh[2]
            else:
                self.xoffset = 0
                self.yoffset = 0
